...
#  account_capital_gains_treatment | capital_gains_treatment     | NOT NULL
#  amount                          | numeric                     | NOT NULL
#  cost_amount                     | numeric                     | NOT NULL
#  sold_amount                     | numeric                     | NOT NULL
#  net_amount                      | numeric                     | NOT NULL
#  account_cost_amount             | numeric                     | NOT NULL DEFAULT 0
#  account_net_amount              | numeric                     | NOT NULL DEFAULT 0
...
class Disposal < Sequel::Model
  ...
  def account_acquired_on
    account_acquired_at&.to_date
  end
  ...
  def set_net_amounts
    ...
    self.net_amount = sold_amount - cost_amount if cost_amount
    self.account_net_amount = sold_amount - account_cost_amount if account_cost_amount
  end

  def set_captital_gains_treatment
    return unless disposed_at

    self.capital_gains_treatment = ((disposed_on - acquired_on).to_i < LONG_TERM_GAIN_DAYS ? 'short_term' : 'long_term') if acquired_at
    return unless account_acquired_at

    self.account_capital_gains_treatment = ((disposed_on - account_acquired_on).to_i < LONG_TERM_GAIN_DAYS ? 'short_term' : 'long_term')
  end
